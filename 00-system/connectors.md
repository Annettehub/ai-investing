...
**用途：** 把 `02-kb/` 作为知识库问答底座，提供检索增强（RAG）能力。

### 6.1 配置方式

1. 在 IMA 创建知识库项目
2. 将 `02-kb/` 下的 markdown 文件批量导入 IMA
3. 或使用 WorkBuddy 的 `ima-mcp` connector 直接读写
4. 在研究时直接问 IMA：「中芯国际的 H1 假设当前证据有哪些？」

### 6.2 推荐用法

- **问答入口**：把 IMA 作为移动端快速查询入口
